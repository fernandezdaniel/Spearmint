...
class MESMCNAV(AbstractAcquisitionFunction):
    ...
    def acquisition(self, obj_models_dict, con_models_dict, cand, current_best, compute_grad, minimize=True, tasks=None, tasks_values=None):
        ...
        for model in models:
            if not model.options['caching']:
                logging.error("Warning: caching is off while using MESMOC_naive!")
        ...
        # tiempo_ini_sin_contar_pareto = time.time()
        NUM_SAMPLES = len(pareto_fronts)
        TAM_CAND = len(cand)
        C = len(con_models_dict)
        K = len(obj_models_dict)

        m_f, v_f, e_f = get_statistics_in_cand(cand, obj_models_dict)
        m_c, v_c, e_c = get_statistics_in_cand(cand, con_models_dict)

        acq_dict = dict()
        keys_f = obj_models_dict.keys()
        keys_c = con_models_dict.keys()
        keys_f_c = keys_f + keys_c
        for key in keys_f_c:
            acq_dict[ key ] = 0.0

        # import pdb;pdb.set_trace()
        for pareto_front in pareto_fronts.values():
            ys_star = get_ys_star(pareto_front)
            # means = np.vstack((m_f, m_c))
            stds_f = np.sqrt(v_f + e_f)

            # import pdb; pdb.set_trace()
            for key_f, mean, std in zip(keys_f, m_f, stds_f):
                gamma = (ys_star[ key_f ] - mean)/std
                Z = sps.norm.sf(gamma)       # 1 - sps.norm.cdf(gamma) <- survival function is this but robust
                Z[np.where(Z < 1e-8)] = 1e-8 # we try to eliminate the zeros
                acq_dict[ key_f ] += -(gamma * sps.norm.pdf(gamma)) / (2 * Z) - sps.norm.logsf(gamma)

                if len(np.where(Z == 0.0)[0] > 0):
                    import pdb;pdb.set_trace()
                    print "Hay ceros en Z_f"

            stds_c = np.sqrt(v_c + e_c)
            for key_c, mean, std in zip(keys_c, m_c, stds_c):
                gamma = (ys_star[ key_c ] - mean)/std
                Z = sps.norm.cdf(gamma)
                Z[np.where(Z < 1e-8)] = 1e-8 # we try to eliminate the zeros
                acq_dict[ key_c ] += (gamma * sps.norm.pdf(gamma)) / (2 * Z) - sps.norm.logcdf(gamma)

                if len(np.where(Z == 0.0)[0] > 0):
                    import pdb;pdb.set_trace()
                    print "Hay ceros en Z_c"


        for key in keys_f_c:
            acq_dict[ key ] /= NUM_SAMPLES
            # acq_dict[ key ] = acq_dict[ key ].clip(min=0) # the minimum adquisition value is zero

        # By default, sum the MESMOC_naive contribution for all tasks
        if tasks is None:
            tasks = acq_dict.keys()
...
def get_ys_star(pareto_front):
    ys_star = dict()

    for name_obj, values_obj in pareto_front['objectives'].items():
        ys_star[name_obj] = np.min(values_obj)

    for name_con, values_con in pareto_front['constraints'].items():
        ys_star[name_con] = np.max(values_con)
