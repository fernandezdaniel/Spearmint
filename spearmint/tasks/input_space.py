...
class InputSpace(object):
    ...
    def __init__(self, variables_config):
        ...
        for name, variable in variables_config.iteritems():
		    ...
		    #vdict['indices'].append(num_dims) #Just treat enum as the rest of types, for the moment.
                    #num_dims += 1
    ...
    def paramify_and_print(self, data_vector, left_indent=0, indent_top_row=True):
	    ...
	    if param['type'] != 'enum':
	    	...
	    	logging.info(format_str % (indentation, param_name, param['type'], param['label']))
    ...
    def paramify_single(self, data_vector):
		...
		#This behaviour was wrong, it was kinda splitting the variables, better only show the real value of the cat. variable.
		#i=0
                #for ind in indices[0]:
                #    params[name + '_' + vdict['options'][i]] = {}
                #    params[name + '_' + vdict['options'][i]]['type'] = vdict['type']
                #    params[name + '_' + vdict['options'][i]]['values'] = np.array([data_vector[ind]])
		#    params[name + '_' + vdict['options'][i]]['index'] = ind
		#    i+=1
		if name not in params:
			params[name] = {}
			params[name]['type'] = vdict['type']
			params[name]['values'] = np.array([])
		#intervals =  np.linspace(0,1,len(vdict['options']))
		#index_value = np.argmin(np.absolute(data_vector[indices[0]] - intervals))
		params[name]['values'] = np.append(params[name]['values'],data_vector[indices[0]])
		if len(params[name]['values']) == len(vdict['options']): #Enum is filled with all options, can now compute the label
			params[name]['label'] = vdict['options'][np.argmax(params[name]['values'])]
		#params[name]['label'] = vdict['options'][index_value]
		#max_cat_value = np.argmax(data_vector[indices[0]])
		#params[name]['one_hot_encoding'] = np.array([1.0 if i == np.max(data_vector[indices[0]]) else 0.0 for i in data_vector[indices[0]]])
		#params[name]['values'] = np.array([np.linspace(0,1,data_vector[indices[0]].shape[0])[np.argmax(data_vector[indices[0]])]])
    ...
    def paramify_multiple(self, data_vector):
		...
		#    params[name + '_' + vdict['options'][i]] = {}
                #    params[name + '_' + vdict['options'][i]]['type'] = vdict['type']
                #    params[name + '_' + vdict['options'][i]]['values'] = np.array([data_vector[0][ind]])
		#    i+=1
		if name not in params:
                	params[name] = {}
                	params[name]['type'] = vdict['type']
			params[name]['values'] = np.array([])
		#intervals =  np.linspace(0,1,len(vdict['options']))
		#index_value = np.argmin(np.absolute(data_vector[0][indices[0]] - intervals))
		#params[name]['values'] = np.array([intervals[index_value]])
		params[name]['values'] = np.append(params[name]['values'],data_vector[0][indices[0]])
		if len(params[name]['values']) == len(vdict['options']): #Enum is filled with all options, can now compute the label.
                	params[name]['label'] = vdict['options'][np.argmax(params[name]['values'])]
		#params[name]['label'] = vdict['options'][index_value]
                #max_cat_value = np.argmax(data_vector[0][indices[0]])
		#params[name]['one_hot_encoding'] = np.array([1.0 if i == np.max(data_vector[0][indices[0]]) else 0.0 for i in data_vector[0][indices[0]]])
		#params[name]['label'] = vdict['options'][max_cat_value]
		#params[name]['values'] = np.array([np.linspace(0,1,data_vector[0][indices[0]].shape[0])[np.argmax(data_vector[0][indices[0]])]])
            else:
                raise Exception('Unknown parameter type.')
    ...
    def vectorify_single(self, params):
		...
		#I do not understand this two lines onder 1-hot encoding enum treatment of categorical values.
		#name = name.partition("_")[0]
        	#v[param['index']] = param['values']
		indices = self.variables_meta[name]['indices']
                v[indices] = param['values']
            else:
                raise Exception('Unknown parameter type.')
    ...
    def from_unit(self, U):
		...
		V[:, indices] = U[:, indices] #Copy values.
                #for ind in indices:
