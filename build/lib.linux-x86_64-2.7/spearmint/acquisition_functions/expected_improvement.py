...
EI_OPTION_DEFAULTS  = {
    'ei_best_model' : False,
    'ei_best_observed' : False,
    'ei_best_sampled' : False,
    'ei_n_samples' : 10
    }
...
class ExpectedImprovement(AbstractAcquisitionFunction):
	...
	self.options = EI_OPTION_DEFAULTS.copy()
	...
	BEST_MODEL = self.options['ei_best_model']
	BEST_OBSERVED = self.options['ei_best_observed']
	BEST_SAMPLED = self.options['ei_best_sampled']
    ...
    def acquisition(self, objective_model_dict, constraint_models_dict, cand, current_best, compute_grad, tasks=None, tasks_values = None):
	    ...
	    if BEST_MODEL == False and BEST_OBSERVED == False and BEST_SAMPLED == False:

	            return compute_ei(objective_model, cand, ei_target=current_best, compute_grad=compute_grad)
	    else:
                    return self.compute_ei_options(objective_model, cand, compute_grad)
        else:
            return constraint_weighted_ei(objective_model, constraint_models_dict.values(), cand, current_best, compute_grad)

    ##
    # Done to test different methods for computing the EI

    def compute_ei_options(self, model, pred, compute_grad):

	# We choose the best according to the options

	assert BEST_MODEL == True or BEST_OBSERVED == True or BEST_SAMPLED == True

	key = model.state

	if not key in self.cached_information:

		# We find the best according to the preferences 

		if BEST_MODEL == True:
			self.cached_information[ key ] = np.min(model.predict(model.inputs)[ 0 ])
	                return compute_ei(model, pred, ei_target = self.cached_information[ key ], compute_grad = compute_grad)
		elif BEST_OBSERVED == True:
			self.cached_information[ key ] = np.min(model.values)
	                return compute_ei(model, pred, ei_target = self.cached_information[ key ], compute_grad = compute_grad)
		elif BEST_SAMPLED == True: 
			
			samples = model.sample_from_posterior_given_hypers_and_data(model.inputs, n_samples = self.options['ei_n_samples'])
			samples = samples.reshape((model.inputs.shape[ 0 ], self.options['ei_n_samples']))

			if model.inputs.shape[ 0 ] > 1:
				self.cached_information[ key ] = np.min(samples, axis = 0)
			else:
				self.cached_information[ key ] = samples[ 0, : ]

			if compute_grad == True:
				ei, grad = compute_ei(model, pred, ei_target = self.cached_information[ key ][ 0 ], compute_grad = compute_grad)
			else:
				ei = compute_ei(model, pred, ei_target = self.cached_information[ key ][ 0 ], compute_grad = compute_grad)

			for i in range(self.options['ei_n_samples'] - 1):
				if compute_grad == True:
					result = compute_ei(model, pred, ei_target = self.cached_information[ key ][ i + 1 ], \
			...
			if compute_grad == True:
				return ei / self.options['ei_n_samples'], grad / self.options['ei_n_samples']
			else:
				return ei / self.options['ei_n_samples']
	else: 
