...
def test_grid():
    ...
    c1 = 225.0-np.power(x,2.0)-np.power(y,2.0)
    c2 = 3.0*y-x-10.0
...
def c1(grid):
	return ((225.0-np.power(grid[:,0],2.0)-np.power(grid[:,1],2.0)+np.random.normal(0,0.04))/168.823484412)*-1.0

def c2(grid):
	return ((3.0*grid[:,1]-grid[:,0]-10.0+np.random.normal(0,0.04))/36.5330991522)*-1.0

def get_functions_borders(num_vars = 2, grid_size = 1000000, noise = 0.01):
