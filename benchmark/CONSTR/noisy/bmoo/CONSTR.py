...
def evaluate(job_id, params):
    ...
    obj1 = float(x) + np.random.normal(0,0.008)
    obj2 = float((1.0+y)/x) + np.random.normal(0,0.58)

    c1 = (float(y+9.0*x-6.0) + np.random.normal(0,0.04)) / 2.74925225179
    c2 = (float(-y+9.0*x-1.0) + np.random.normal(0,0.04)) / 2.74925225179
...
def test_grid():
    x = np.linspace( 0.1,1.0,2000 )
    y = np.linspace( 0.0,5.0,2000 )
    x,y = np.meshgrid( x,y )
...
def c1(grid):
	return (((grid[:,1]+9.0*grid[:,0]-6.0)+np.random.normal(0,0.04))/2.74925225179)*-1.0

def c2(grid):
	return (((-grid[:,1]+9.0*grid[:,0]-1.0)+np.random.normal(0,0.04))/2.74925225179)*-1.0
