...
def evaluate(job_id, params):
    ...
    obj1 = float(106780.37*(y+z) + 61704.67)
    obj2 = float(3000.0*x)
    obj3 = 305700.0*2289.0*y/np.power(0.06*2289.0,0.65)
    obj4 = 250.0*2289.0*np.exp(-39.75*y + 9.9*z + 2.74)
    obj5 = 25.0*(1.39/(x*y)+4940.0*z-80.0)

    c1 = float(1.0-0.00139/(x*y)-4.94*z+0.08)/0.636279333822
    c2 = float(1.0-0.000306/(x*y)-1.082*z+0.0986)/0.140043905079
    c3 = float(50000.0-12.307/(x*y) - 49408.24*z - 4051.02)/5665.45611369
    c4 = float(16000.0 - 2.098/(x*y) - 8046.33*z + 696.71)/963.611681082
    c5 = float(10000.0 - 2.138/(x*y) - 7883.39*z + 705.04)/980.215853013
    c6 = float(2000.0 - 0.417/(x*y) - 1721.26*z + 136.54 )/192.247633347
    c7 = float(550.0 - 0.164/(x*y) - 631.13*z + 54.48 )/75.3374871091
...
def test_grid():
    ...
    y = np.linspace( 0.01,0.1,200 )
    z = np.linspace( 0.01,0.1,200 )

    x,y,z = np.meshgrid( x,y,z )

    c1 = 1.0-0.00139/(x*y)-4.94*z+0.08
    c2 = 1.0-0.000306/(x*y)-1.082*z+0.0986
    c3 = 50000.0-12.307/(x*y) - 49408.24*z - 4051.02
    c4 = 16000.0 - 2.098/(x*y) - 8046.33*z + 696.71
    c5 = 10000.0 - 2.138/(x*y) - 7883.39*z + 705.04
    c6 = 2000.0 - 0.417/(x*y) - 1721.26*z + 136.54
    c7 = 550.0 - 0.164/(x*y) - 631.13*z + 54.48
...
def c1(grid):
	return ((1.0-0.00139/(grid[:,0]*grid[:,1])-4.94*grid[:,2]+0.08)/0.636279333822)*-1.0

def c2(grid):
	return ((1.0-0.000306/(grid[:,0]*grid[:,1])-1.082*grid[:,2]+0.0986)/0.140043905079)*-1.0

def c3(grid):
	return ((50000.0-12.307/(grid[:,0]*grid[:,1]) - 49408.24*grid[:,2] - 4051.02)/5665.45611369)*-1.0

def c4(grid):
	return ((16000.0 - 2.098/(grid[:,0]*grid[:,1]) - 8046.33*grid[:,2] + 696.71)/963.611681082)*-1.0

def c5(grid):
	return ((10000.0 - 2.138/(grid[:,0]*grid[:,1]) - 7883.39*grid[:,2] + 705.04)/980.215853013)*-1.0

def c6(grid):
	return ((2000.0 - 0.417/(grid[:,0]*grid[:,1]) - 1721.26*grid[:,2] + 136.54)/192.247633347)*-1.0

def c7(grid):
	return ((550.0 - 0.164/(grid[:,0]*grid[:,1]) - 631.13*grid[:,2] + 54.48)/75.3374871091)*-1.0
...
if __name__ == "__main__":
	#main(0, {u'X': np.array([ 0.2 ]), u'Y': np.array([ 0.05 ]), u'Z': np.array([ 0.05 ]) })
