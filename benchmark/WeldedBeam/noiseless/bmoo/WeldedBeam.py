...
def test_grid():
    x = np.linspace( 0.125,5.0,75 )
    y = np.linspace( 0.125,5.0,75 )
    z = np.linspace( 0.1,10.0,75 )
    a = np.linspace( 0.1,10.0,75 )

    x,y,z,a = np.meshgrid( x,y,z,a )

    c1 = 13600.0-np.power(np.power(6000.0/(np.power(2,0.5)*x*z),2.0)+ np.power( (6000.0*(14.0+0.5*z)*np.power(0.25*(np.power(z,2.0)+np.power(x+a,2.0)),0.5)/(2*np.power(2.0,0.5)*x*z*(np.power(z,2.0)/(12.0)+0.25*np.power(x+a,2.0)))) ,2.0) + z * 6000.0/(np.power(2,0.5)*x*z) * ((6000.0*(14.0+0.5*z)*np.power(0.25*(np.power(z,2.0)+np.power(x+a,2.0)),0.5)/(2.0*np.power(2.0,0.5)*x*z*(np.power(z,2.0)/(12.0)+0.25*np.power(x+a,2.0))))) / (np.power(0.25*(np.power(z,2.0)+np.power(x+a,2.0)),0.5)),0.5)
    c2 = 30000.0-504000/(np.power(a,2.0)*y)
    c3 = y - x
    c4 = 64746.022 * (1.0 - 0.0282346 * a) * a *np.power(y, 3.0) - 6000.0
...
def c1(grid):
	return ((13600.0-np.power(np.power(6000.0/(np.power(2,0.5)*grid[:,0]*grid[:,2]),2.0)+ np.power( (6000.0*(14.0+0.5*grid[:,2])*np.power(0.25*(np.power(grid[:,2],2.0)+np.power(grid[:,0]+grid[:,3],2.0)),0.5)/(2*np.power(2.0,0.5)*grid[:,0]*grid[:,2]*(np.power(grid[:,2],2.0)/(12.0)+0.25*np.power(grid[:,0]+grid[:,3],2.0)))) ,2.0) + grid[:,2] * 6000.0/(np.power(2,0.5)*grid[:,0]*grid[:,2]) * ((6000.0*(14.0+0.5*grid[:,2])*np.power(0.25*(np.power(grid[:,2],2.0)+np.power(grid[:,0]+grid[:,3],2.0)),0.5)/(2*np.power(2.0,0.5)*grid[:,0]*grid[:,2]*(np.power(grid[:,2],2.0)/(12.0)+0.25*np.power(grid[:,0]+grid[:,3],2.0))))) / (np.power(0.25*(np.power(grid[:,2],2.0)+np.power(grid[:,0]+grid[:,3],2.0)),0.5)),0.5))/75842.5359709)*-1.0

def c2(grid):
	return ((30000.0-504000/(np.power(grid[:,3],2.0)*grid[:,1]))/8526363.04783)*-1.0

def c3(grid):
	return ((grid[:,1] - grid[:,0])/2.01692584516)*-1.0

def c4(grid):
	return ((64746.022 * (1.0 - 0.0282346 * grid[:,3]) * grid[:,3] *np.power(grid[:,1], 3.0) - 6000.0)/11617706.4105)*-1.0

def get_functions_borders(num_vars = 4, grid_size = 100000):
